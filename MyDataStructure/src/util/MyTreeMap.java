...
public class MyTreeMap<K,V> implements MyNavigableMap<K,V> {
...
//        super.putAll(map);
    ...
    // Comparator가 없는 경우
    // 있는데 호출하면 getEntryUsingComparator 호출
    ...
    // 함수 설명은 MySortedMap 주석 확인
    // 아니면 TreeMap 들어가서 다시 확인
    @Override
    public MyMap.Entry<K,V> lowerEntry(K key) { return exportEntry(getLowerEntry(key));  }
    @Override
    public K lowerKey(K key) { return keyOrNull(getLowerEntry(key));}
    @Override
    public MyMap.Entry<K,V> floorEntry(K key) { return exportEntry(getFloorEntry(key)); }
    @Override
    ...
    class Values extends AbstractCollection<V> {
        ...
        public Spliterator<V> spliterator() {
            return new TreeMap.ValueSpliterator<K,V>(this, null, null, 0, -1, 0);
        }
    }
    class EntrySet extends AbstractSet<Map.Entry<K,V>> {
        ...
        public Spliterator<Map.Entry<K,V>> spliterator() {
            return new EntrySpliterator<K,V>(this, null, null, 0, -1, 0);
        }
    }

    // values 및 EntrySet과 달리 KeySet은 정적 클래스 submap에서도 사용 가능
    ...
    abstract static class NavigableSubMap<K,V> extends AbstractMap<K,V>
        ...
        abstract class SubMapIterator<T> implements Iterator<T> {
            ...
            SubMapIterator(Entry<K,V> first,
                           Entry<K,V> fence) {
        ...
        final class DescendingSubMapEntryIterator extends TreeMap.NavigableSubMap.SubMapIterator<Entry<K,V>> {
            DescendingSubMapEntryIterator(Entry<K,V> last,
                                          Entry<K,V> fence) {
                super(last, fence);
            }
        ...
        final class SubMapKeyIterator extends NavigableSubMap.SubMapIterator<K>
                implements Spliterator<K> {
            SubMapKeyIterator(Entry<K,V> first,
                              Entry<K,V> fence) {
                super(first, fence);
            }
            public K next() {
        ...
        final class DescendingSubMapKeyIterator extends NavigableSubMap.SubMapIterator<K>
                implements Spliterator<K> {
            DescendingSubMapKeyIterator(Entry<K,V> last,
                                        Entry<K,V> fence) {
                super(last, fence);
            }
            public K next() {
    ...
    static final class AscendingSubMap<K,V> extends NavigableSubMap<K,V> {
        ...
        Entry<K,V> subLowest()       { return absLowest(); }
        Entry<K,V> subHighest()      { return absHighest(); }
        Entry<K,V> subCeiling(K key) { return absCeiling(key); }
        Entry<K,V> subHigher(K key)  { return absHigher(key); }
        Entry<K,V> subFloor(K key)   { return absFloor(key); }
        Entry<K,V> subLower(K key)   { return absLower(key); }
    }
    ...
    static final class DescendingSubMap<K,V>  extends NavigableSubMap<K,V> {
        ...
        Entry<K,V> subLowest()       { return absHighest(); }
        Entry<K,V> subHighest()      { return absLowest(); }
        Entry<K,V> subCeiling(K key) { return absFloor(key); }
        Entry<K,V> subHigher(K key)  { return absLower(key); }
        Entry<K,V> subFloor(K key)   { return absCeiling(key); }
        Entry<K,V> subLower(K key)   { return absHigher(key); }
    }
